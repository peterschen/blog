...
resource "google_compute_instance" "wsfc" {
  count = local.node_count
  project = data.google_project.default.project_id
  ...
  dynamic "attached_disk" {
    ...
    content {
      ...
      device_name = "wsfc-${count.index}-capacity-${floor((attached_disk.value - count.index) / local.node_count)}"
    }
  }
  ...
  metadata = {
    ...
    sysprep-specialize-script-ps1 = templatefile(module.sysprep.path_specialize, { 
        nameHost = "wsfc-${count.index}", 
        password = local.password,
        parametersConfiguration = jsonencode({
          inlineMeta = filebase64(module.sysprep.path_meta),
          inlineConfiguration = filebase64("${path.module}/wsfc.ps1"),
          inlineConfigurationCustomization = base64encode(local.configuration_customization),
...
resource "google_compute_instance_group" "cluster" {
  count = length(google_compute_instance.wsfc)
  project = data.google_project.default.project_id
  zone = local.cluster_zones[count.index]
  ...
  instances = [
    google_compute_instance.wsfc[count.index].self_link
  ]
}
